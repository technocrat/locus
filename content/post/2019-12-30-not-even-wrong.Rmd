...
my_model_plot <- p + geom_point() + geom_abline(slope = slope) + annotate("text", x = 0.15, y = -0.25, label = sources, hjust = 0) + annotate("text", x = -0.25, y = -0.2, label = rsq) + labs(x = x_lab, y = y_lab) + theme_economist() + ggtitle(heading)
```
