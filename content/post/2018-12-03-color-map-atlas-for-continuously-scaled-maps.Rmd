...
suppressPackageStartupMessages(library(scales)) # needed for label = comma
...
q2_2018_gdp <- as_tibble(read.csv("data/Q2_2018_GDP.csv", header = FALSE, stringsAsFactors = FALSE)) %>% rename(NAME = V1, Q22018_GDP = V2)
...
gdp_pop <- left_join(gdp_table, states, by = "NAME") %>% select(NAME, id, GDP, clong, clat, offset_clong, offset_clat, geometry)
