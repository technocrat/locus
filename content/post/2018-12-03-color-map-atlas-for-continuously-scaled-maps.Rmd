...
converter <-  read.csv(here("content/post/data/state_fips_postal.csv"), header = FALSE, stringsAsFactors = FALSE)
converter <-  rename(converter, NAME = V1, geoid = V2, id = V3)
states_key <- converter %>% filter(id != 'DC')
...
states_centroid <- as_tibble(states_centroid_matrix) %>% transmute(clong = X, clat = Y)
# construct a key
geoid <- as_tibble(states_pop$geoid)
colnames(geoid) <- "geoid"
# combine to tibble
states_centroid <- cbind(geoid, states_centroid)
# bring into states tibble and create columns for position adjustment

states <- left_join(states_pop, states_centroid, by = "geoid") %>% mutate(offset_clong = 0, offset_clat = 0)
# abbreviations
states <- left_join(states,converter, by = "NAME")
...
q2_2018_gdp <- as_tibble(read.csv(here("content/post/data/Q2_2018_GDP.csv"), header = FALSE, stringsAsFactors = FALSE)) %>% rename(NAME = V1, Q22018_GDP = V2)
# econ <- left_join(q2_2018_gdp, states, by = "NAME") %>% mutate(GDP = Q22018_GDP) %>% mutate(capita = GDP/(POP_TOT/1e5)) %>% select(NAME, id, GDP, capita, clong, clat, offset_clong, offset_clat, geometry)
econ <- inner_join(states, q2_2018_gdp,by = "NAME") %>% mutate(GDP = Q22018_GDP) %>% mutate(capita = GDP/(POP_TOT/1e5)) %>% select(NAME, id, GDP, capita, clong, clat, offset_clong, offset_clat, geometry)
...
gdp_pop <- st_join(states,gdp_table, by = "NAME") %>% select(NAME.x, geoid.x, GDP, clong, clat, offset_clong, offset_clat, geometry)
